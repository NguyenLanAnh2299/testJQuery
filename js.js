$(document).ready(function(dataJson){
	var dataJson = {
    "users": [
      {
        "id": 1,
	...
	$.each(dataJson.offices, function(sm, data){
		off += '<option data-v="'+data.id+'"  value="' + data.id + '">' + data.name + '</option>'; 
	});
	$.each(dataJson.positions, function(sm, data){
		po += '<option data-v="'+data.id+'"  value="' + data.id + '">' + data.name + '</option>'; 
	});
	...
	$('#blog_table tbody').html(dataJson);

	$('.select1').append(off);
	$('.select2').append(po);
	// find arr selete 
	$(this).find('.select1').each(function(s){
		var id = $(this).attr('data-id');
		var val = $(this).val(id);
		val.find("option[value=" + id +"]").attr('selected', true);
	});
	$(this).find('.select2').each(function(s){
	...
	$("#blog_table").on('click', 'tr', function getDataTable(){
		...
		$('#detailName').val(name);
		$('#detailPosition').val(position);
		$('#detailSalary').val(salary);
		$('#detailDate').val(start_date);
